...
def is_permanent_api_error(exc: Exception) -> bool:
    """Return True if exc is a permanent 'API not enabled' error."""
    s = str(exc)
    return any(m in s for m in _PERMANENT_GCP_MARKERS)
...
def friendly_error(raw: Exception | str) -> str:
    ...
    if any(m in s for m in _QUOTA_MARKERS) or "quota" in s.lower():
        return (
            ...
            "request an increase in GCP Console → IAM & Admin → Quotas."
        )

    if "403" in s and any(k in s for k in ("PERMISSION_DENIED", "Forbidden", "PermissionDenied")):
        return "Permission denied — check that your account has the required IAM roles."
